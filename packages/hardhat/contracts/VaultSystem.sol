...
contract VaultSystem {
    ...
    function depositToVault(
        VaultType _vaultType,
        uint256 _amount
    ) external payable validVaultType(_vaultType) validAmount(_amount) {
    ...
    function withdrawFromVault(
        VaultType _vaultType,
        uint256 _amount
    ) external validVaultType(_vaultType) validAmount(_amount) hasVault(msg.sender, _vaultType) {
        UserVault storage vault = userVaults[msg.sender][_vaultType];
    ...
    function executeAutoDeposit(
        VaultType _vaultType
    ) external payable validVaultType(_vaultType) hasVault(msg.sender, _vaultType) {
        UserVault storage vault = userVaults[msg.sender][_vaultType];
    ...
    function calculateYield(
        address _user,
        VaultType _vaultType
    ) external validVaultType(_vaultType) hasVault(_user, _vaultType) {
    ...
    {
        return (totalUsers, totalDeposited, totalWithdrawn, totalYieldEarned, totalInsurancePolicies, protocolFeeRate);
    }
