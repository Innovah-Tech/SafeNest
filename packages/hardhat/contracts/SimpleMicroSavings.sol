...
contract SimpleMicroSavings {
     ...
     */
    function depositSavings(uint256 _amount) external payable validAmount(_amount) {
    ...
    function withdrawSavings(uint256 _amount) external validAmount(_amount) {
        require(userSavingsAccounts[msg.sender].currentBalance >= _amount, "Insufficient balance");
     ...
     */
    function investInStrategy(uint256 _strategyId, uint256 _amount) external payable validAmount(_amount) {
     ...
     */
    function depositEmergencyFund(uint256 _amount) external payable validAmount(_amount) {
        require(msg.value >= _amount, "Insufficient ETH sent");
